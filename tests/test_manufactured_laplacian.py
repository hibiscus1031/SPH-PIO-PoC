"""Manufactured periodic scalar-Laplacian verification."""
...
from manufactured_fields.periodic import scalar_field, scalar_laplacian
...
def test_manufactured_laplacian_has_overall_refinement_trend(
    ...
    for resolution in (16, 24, 32):
        ...
        numerical = apply_laplacian(
            context,
            neighborhood,
            scalar_field(positions),
        )
        errors.append(error_norms(numerical, scalar_laplacian(positions)))

    for norm in ("l1", "l2"):
        assert errors[-1][norm] < errors[0][norm]


def test_constant_scalar_laplacian_is_zero() -> None:
    ...
    numerical = apply_laplacian(context, neighborhood, constant)
