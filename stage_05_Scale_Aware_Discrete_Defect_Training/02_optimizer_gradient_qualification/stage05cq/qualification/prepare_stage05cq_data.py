"""After Stage 05C-Q freeze, verify TRAIN targets and cache the blind 48-origin batch."""
...
def import_access() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("stage05cq_access", path)
...
def denial(phase: str) -> dict[str, Any]:
    ...
    for kind, path in probes.items():
        try:
            ACCESS.read_bytes(path); denied = False
    ...
    write_json(STAGE05CQ / f"access_control/{phase}_allowlist_denial_audit.json", result)
    return result
...
def make_state(arrays: dict[str, np.ndarray], resolution: int, frame: int) -> DynamicParticleState:
    index = int(np.flatnonzero(arrays["frame_n"] == frame)[0])
    density = tensor(arrays["density"][index]); dx = L / resolution
    return DynamicParticleState(tensor(arrays["position_unwrapped"][index]), tensor(arrays["velocity"][index]), density, eos_pressure(density),
...
def main() -> None:
    freeze = json.loads((STAGE05CQ / "freeze/stage05cq_freeze_record.json").read_text())
    ...
    for entry in target_manifest["records"]:
        ...
        if key in selected:
            ...
            target_values[key] = {"a_cons": arrays["a_cons"], "a_def": arrays["a_def"], "y_def": arrays["y_def"], "metadata": metadata}
    zero_loss = float(np.mean(zero_losses))
    assert abs(zero_loss - 1) <= 1e-12 and len(target_values) == 48

    cache_dir = STAGE05CQ / "blind_origin_selection/case_cache"; cache_dir.mkdir(parents=True, exist_ok=True)
    cache = []
    for row in origins["selection"]:
        ...
        for origin in row["origins"]:
            ...
            frames = list(range(origin - 3, origin + 1)); states = [make_state(trajectory, 8, frame) for frame in frames]
            ...
            np.savez_compressed(path, frames=np.asarray(frames), physical_times=np.asarray([state.physical_time for state in states]),
                                ...
                                mass=states[-1].mass.numpy(), smoothing=states[-1].smoothing_length.numpy(), history_tokens=history_tokens,
            ...
            cache.append({"record_id": record_id, "lineage": lineage, "variant": variant, "origin": origin,
                          "path": str(path.relative_to(ROOT)), "sha256": sha_file(path),
                          "target_canonical_sha256": target["metadata"]["canonical_sha256"]})
    result = {"schema": "sph-pio-poc.stage05cq.cached-blind-batch.v1", "contract_sha256": freeze["contract_sha256"],
              "case_count": len(cache), "zero_correction_baseline_all384": zero_loss,
              "zero_correction_absolute_error": abs(zero_loss - 1), "cases": cache, "decode_counts": DECODE,
              "target_values_enter_tokens": False, "stage05c_origin_overlap_count": 0,
              "pass": len(cache) == 48 and abs(zero_loss - 1) <= 1e-12}
    write_json(STAGE05CQ / "blind_origin_selection/cached_blind_batch_manifest.json", result)
    print(json.dumps({"cases": len(cache), "zero_loss": zero_loss, "decode_counts": DECODE}))
