"""Stage 06A TRAIN-only dual-root allowlist."""
...
TRAJECTORY_ROOT = (STAGE04B / "exact_trajectories/train").resolve()
...
TRAJECTORY = re.compile(r"^(lcdf_\d{2})_(variant_(?:low|main))_n8\.(npz|json)$")
...
def _authorized(path: str | Path) -> Path:
    ...
    if resolved.is_relative_to(TARGET_ROOT):
        ...
        if match and match.group(1).lower() in ALLOWED and int(match.group(3)) < 32:
            return resolved
    raise PermissionError("Stage06A access denied before payload read")
...
def load_npz(path: str | Path) -> dict[str, Any]:
    with np.load(io.BytesIO(read_bytes(path)), allow_pickle=False) as archive:
