    ...
    for family in ("constant_neighbor","increasing_neighbor"):
        for layout in ("regular","jitter_05","jitter_10"):
            for n in (16,24,32,48,64):
                ...
                for key in ("density_rms","EOS_pressure_rms","analytic_pressure_rms","R_pressure_operator_L2","R_EOS_initialization_L2","R_viscosity_L2","R_total_L2","closure_Linf","mean_edge_count"):
...
def main()->int:
    ...
    d2_regular=read_json(D2/"run_summaries"/"stage01d2_dis_regular.json"); d2_jitter=[read_json(path) for path in sorted((D2/"run_summaries").glob("stage01d2_dis_j*.json"))]
    resource_explanation={"short_spearman_relative_vs_baseline":float(spearmanr(relative,baseline).statistic),"short_spearman_absolute_vs_edge_change":float(spearmanr(absolute,edges).statistic) if len(np.unique(edges))>1 else None,"short_median_absolute_rss_increase_bytes":float(np.median(absolute)),"short_median_relative_rss_increase":float(np.median(relative)),"stage01d2_jitter_median_first_quartile_rss_bytes":statistics.median(float(x["first_quartile_rss_median_bytes"]) for x in d2_jitter),"stage01d2_regular_first_quartile_rss_bytes":d2_regular["first_quartile_rss_median_bytes"],"stage01d2_jitter_median_absolute_rss_increase_bytes":statistics.median(float(x["quartile_rss_increase_bytes"]) for x in d2_jitter),"stage01d2_regular_absolute_rss_increase_bytes":d2_regular["quartile_rss_increase_bytes"],"stage01d2_jitter_median_edge_count":statistics.median(float(x["mean_edge_count"]) for x in d2_jitter),"stage01d2_regular_edge_count":d2_regular["mean_edge_count"],"mechanism_judgment":"sustained absolute working-set growth over the 400-step jitter horizon, with denominator amplification; not a lower initial-RSS denominator and not more edges","interpretation":"auxiliary evidence only; no Stage 01D2 resource reclassification"}
...
def generate_reports(cfg:dict,e:dict,all_runs:list[dict],facts:dict[str,list[dict]],summaries:list[dict],trends:list[dict],correlations:list[dict],growth:list[dict],resource_rows:list[dict],model:dict)->None:
    ...
    static_full=read_csv(RESULTS/"initial_residual_matrix.csv"); focus_norms=[]
    for layout in ("regular","jitter_05","jitter_10"):
        selected=[row for row in static_full if row["support_family"]=="increasing_neighbor" and row["resolution"]=="32" and row["layout"]==layout]
        for component in ("pressure_operator","EOS_initialization","viscosity","total"):
            focus_norms.append({"layout":layout,"component":component,**{norm:statistics.mean(float(row[f"R_{component}_{norm}"]) for row in selected) for norm in ("L1","L2","Linf","relative_L2")}})
    ...
    noise_cols=["support_family","layout","N","cases","density_rms_mean","EOS_pressure_rms_mean","analytic_pressure_rms_mean"]
    ...
    short_corr={row["metric"]:row for row in correlations if row["dataset"]=="short_20_frozen_jitter_seeds"}; d2_corr={row["metric"]:row for row in correlations if row["dataset"]=="stage01d2_six_matched_jitter_runs"}
    report("stage_01e_disorder_error_correlation.md",f"# Stage 01E — Disorder/error correlation\n\n{markdown(correlations,list(correlations[0]))}\n\n短程 20 个冻结 jitter seed 中，EOS 初始化/total residual 与 velocity error 的 Spearman rho 均为 `{short_corr['R_EOS_initialization_L2']['spearman_rho']:.4f}`，压力输入算子项为 `{short_corr['R_pressure_operator_L2']['spearman_rho']:.4f}`。Stage 01D2 六条精确匹配轨迹对应 rho 分别为 `{d2_corr['R_EOS_initialization_L2']['spearman_rho']:.4f}` 与 `{d2_corr['R_pressure_operator_L2']['spearman_rho']:.4f}`。因此 10% 的 9.338 放大与 EOS/total residual 伴随最强；5% 与 10% 在 20-seed 短程数据上与同一连续趋势相容。Spearman、Theil–Sen robust slope 与 bootstrap 95% 区间均已报告；相关性不解释为因果，六条 Stage 01D2 种子也不冒充完整随机不确定性。\n")
    endpoints=[row for row in growth if row["step"] in (0,1,5,40)]
    final_sections=[
        ("1. Stage 01D2 冻结",frozen),("2. 形式失败与科学失败", "Stage 01D2 的正式失败由冻结的 50% RSS/无序门触发；Stage 01E 只解释科学机制，不撤销或放宽该形式判定。"),("3. 完整时间、空间、jitter、Mach 数值", f"### 时间\n\n{markdown(facts['time'],list(facts['time'][0]))}\n\n### Increasing（含 N48）\n\n{markdown(facts['increasing'],list(facts['increasing'][0]))}\n\n### Constant\n\n{markdown(facts['constant'],list(facts['constant'][0]))}\n\n### Jitter\n\n{markdown(facts['jitter'],list(facts['jitter'][0]))}\n\n### Mach\n\n{markdown(facts['mach'],list(facts['mach'][0]))}\n\n全部 run 的 H、dx、H/dx、edge count 与 wall time 见结果审计报告。"),("4. TGV 解析压力与材料加速度", "解析压力提供对流平衡，材料加速度恒等式已在随机 float64 坐标测试。"),("5. t=0 残差分解",f"210/210 cases，最大闭合 Linf `{e['maximum_closure_linf']:.6g}`。"),("6. EOS 初始压力不相容",f"N32 increasing jitter 的 EOS/operator 中位 L2 比为 `{e['EOS_to_pressure_operator_ratio']:.6g}`。"),("7. 压力与黏性保守算子残差",f"中位分量 `{e['focus_n32_increasing_jitter_median_components']}`。"),("8. 短程误差增长",f"{markdown(endpoints,['layout','step','time','velocity_relative_l2_mean','density_fluctuation_mean','EOS_pressure_rms_mean','total_material_residual_l2_mean'])}\n\nstep 0 已有密度/EOS/材料残差，但 velocity error 从 0 开始；到第 5 步仍低于终值一半，故 velocity error 分类为逐步累积而非 step-0 已存在或少数 acoustic steps 完成。分类：`{e['short_growth_classification']}`。这些 40-step 轨迹不是新 V2 证据。"),("9. 支撑路径分析",f"{model['status']}；increasing path 不报告标准空间阶。"),("10. 无序相关性",f"短程 EOS/total rho=`{short_corr['R_EOS_initialization_L2']['spearman_rho']:.4f}`，Stage 01D2 六匹配轨迹 rho=`{d2_corr['R_EOS_initialization_L2']['spearman_rho']:.4f}`；5%/10% 与同一连续趋势相容，但无因果声明。"),("11. 资源增量辅助解释",f"{markdown(resource_rows,list(resource_rows[0]))}\n\n`{e['resource_auxiliary']}`。Stage 01D2 jitter 初始 RSS 中位数不低于 regular，edge count 也未增加；主要证据指向 400-step horizon 上持续的绝对 working-set 增长，分母只放大相对百分比。不产生新的资源 PASS。"),("12. 唯一机制分类",f"**{e['unique_classification']}**"),("13. 下一阶段允许路线", "当前分类只允许下一步设计 weakly-compressible/WCSPH-compatible MMS，并停止把未强制的不可压 TGV 当作唯一 exact-reference benchmark。本阶段不实际启动该设计。"),("14. V3 与 Stage 02 边界","V3 与 Stage 02 均未开始；未训练网络、未生成标签、未重跑正式 V2。")]
