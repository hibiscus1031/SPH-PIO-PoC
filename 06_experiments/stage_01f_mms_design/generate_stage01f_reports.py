...
""")
    ...
    sections=[("1. Stage 01E 冻结",frozen),("2. WCSPH 连续方程约定","`dx/dt=u`；`partial_t rho+div(rho u)=0`；`partial_t u+(u·grad)u=-(1/rho)grad(p)+nu laplacian(u)+f_MMS`；`p=c_s^2(rho-rho0)`。`f_MMS` 是单位质量外部加速度；内部守恒与外部作用分开，动量比较 `d sum(m_i u_i)/dt` 与 `sum(m_i f_i)`。"),("3. 公共参数",f"Domain `[-1,1)^2`，rho0={p['rho0']}，c_s={p['sound_speed']}，nu={p['viscosity']}，k=pi，U_ref={p['reference_velocity']}，Ma={p['reference_mach']}，epsilon={p['density_amplitude']}，rho∈[{p['density_minimum']},{p['density_maximum']}]，U_c={p['mms_a_translation_speed']}，lambda={p['mms_b_decay_rate']}，U_v={p['mms_b_velocity_amplitude']}，t∈[0,0.2]。"),("4. MMS-A 完整公式","`xi=x-U_c t`；`rho_A=rho0[1+epsilon sin(k xi)sin(ky)]`；`u_A=[U_c,0]`；`p_A=c_s^2(rho_A-rho0)`。`partial_t rho_A=-rho0 epsilon k U_c cos(k xi)sin(ky)`；`grad rho_A=rho0 epsilon k[cos(k xi)sin(ky),sin(k xi)cos(ky)]`；`div(rho_A u_A)=U_c partial_x rho_A`；`grad p_A=c_s^2 grad rho_A`。material acceleration 与 `laplacian u_A` 为零，`f_A=grad(p_A)/rho_A`。轨迹为 `x_i(t)=wrap(x_i(0)+U_c t)`、`y_i(t)=y_i(0)`。"),("5. MMS-B 完整公式","`psi=sin(kx)sin(ky)`；`rho_B=rho0(1+epsilon psi)`；`a=U_v exp(-lambda t)`；`u_B=[a sin(kx)cos(ky),-a cos(kx)sin(ky)]`；`p_B=c_s^2(rho_B-rho0)`。`div u_B=0`、`u_B·grad rho_B=0`、`partial_t rho_B=0`、`partial_t u_B=-lambda u_B`、`laplacian u_B=-2k^2u_B`；convection=`(a^2k/2)[sin(2kx),sin(2ky)]`；`grad p_B=c_s^2rho0 epsilon k[cos(kx)sin(ky),sin(kx)cos(ky)]`；`f_B=partial_t u_B+convection+grad(p_B)/rho_B-nu laplacian(u_B)`。"),("6. EOS、连续和动量闭合",f"{table(closure,list(closure[0]))}"),("7. Manual/autograd 对照",f"最大差 `{e['manual_autograd_maximum_difference']}`；source 最大差 `{e['source_manual_autograd_maximum_difference']}`，满足 1e-11 门。"),("8. 项尺度审计",f"{table(scales,list(scales[0]))}\n\n最小/最大 L2 比 `{e['minimum_core_term_fraction_of_maximum']}` > 1e-4。"),("9. 粒子质量初始化","规则格点 `V_i^0=(2/N)^2`、`m_i=rho_exact(x_i,y_i,0)V_i^0`；质量固定；MMS-A/B 的解析密度沿各自粒子轨迹不变；不覆盖 numerical kernel density；无 jitter 质量设计。"),("10. Source injection contract","接口按 numerical position 与 physical stage time 在 start/midpoint 分别重算。禁止复用起点 source、analytic position、numerical residual feedback、混入内部 pair antisymmetry 或 internal-force-zero gate；external momentum/power 单独审计。"),("11. MMS-B 独立轨迹参考计划","后续独立求解 `dx/dt=u_exact`，DOP853 或同等级，rtol≤1e-12、atol≤1e-14；使用连续 unwrapped trajectory，仅场评价时 wrap，并做时间步敏感性。Stage 01F 未生成正式轨迹。"),("12. 后续误差指标",f"只定义：{metrics}。必须区分 continuum closure、trajectory-reference error、SPH spatial error、RK2 temporal error 与 forcing discretization error。"),("13. 唯一 MMS 规范状态",f"**{e['status']}**"),("14. Stage 01F2 资格","具备申请 Stage 01F2 设计审计资格。" if e["status"]=="MMS_SPECIFICATION_PASS" else "不具备；需先解决条件或失败。"),("15. V3 和 Stage 02 边界","V3、Stage 02、Stage 01F2、训练和学习标签均未开始。")]
