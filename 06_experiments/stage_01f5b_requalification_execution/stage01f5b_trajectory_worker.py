"""Isolated no-grad RK2 trajectory/smoke worker for Stage 01F5B."""
...
def main() -> int:
    ...
    if not args.run_id.startswith("f5_") or args.run_id.startswith("f5_ref_"):
        raise ValueError("invalid Stage 01F5B trajectory run ID")
    if not gc.isenabled():
        raise RuntimeError("default cyclic GC must remain enabled")
    steps = round(args.t_final / args.dt)
    if steps < 1 or abs(steps * args.dt - args.t_final) > 1.0e-14:
    ...
    if not np.allclose(raw_sample_steps, sample_steps, rtol=0.0, atol=1.0e-12):
        raise ValueError("dt is incompatible with the frozen common-time grid")
    ...
    arrays: dict[str, list[np.ndarray]] = {key: [] for key in ("positions", "unwrapped_positions", "velocities", "densities", "pressures")}
    ...
    def sample() -> None:
        ...
        arrays["unwrapped_positions"].append(unwrapped.numpy().copy())
    ...
    with torch.no_grad():
        ...
        for step in range(1, steps + 1):
            ...
            source_contract = source_contract and abs(result.source_calls[0].physical_time - state.time) <= 1.0e-14
            source_contract = source_contract and abs(result.source_calls[1].physical_time - (state.time + 0.5 * args.dt)) <= 1.0e-14
            ...
            unwrapped = unwrapped + args.dt * result.midpoint_state.velocities
            ...
            if changed:
                ...
                reciprocal_events = reciprocal_events and all(((key % count) * count + (key // count)) in changed for key in changed)
            ...
            if step in sample_lookup:
                sample()
    checkpoint = STAGE / "checkpoints" / f"{args.run_id}.npz"
    if checkpoint.exists():
        raise RuntimeError(f"refusing to overwrite {checkpoint.relative_to(ROOT)}")
    np.savez_compressed(
        checkpoint,
        times=times,
    ...
    time_ratio = statistics.median(step_times[-quarter:]) / max(statistics.median(step_times[:quarter]), 1.0e-30)
    peak_rss = process_peak_rss_bytes()
    maximum_current_rss = max(rss_values)
    gates = yaml.safe_load(CONFIG.read_text())["hard_gates"]
    checks = {
        "state_and_rhs_finite": all_finite and all(np.isfinite(np.stack(values)).all() for values in arrays.values()),
        ...
        "current_rss": maximum_current_rss < gates["current_rss_bytes"],
        "peak_rss": peak_rss < gates["peak_rss_bytes"],
    ...
    if args.role == "conditional_n64_smoke":
        formal_steps = round(0.02 / args.dt)
        checks["smoke_current_rss"] = maximum_current_rss < 2_000_000_000
        checks["smoke_peak_rss"] = peak_rss < 2_000_000_000
        checks["estimated_formal_wall_time"] = statistics.median(step_times) * formal_steps < 7200.0
    payload = {
        "schema_version": "sph-pio-poc.stage01f5b.trajectory-run.v1",
        "run_id": args.run_id,
        "role": args.role,
        ...
        "estimated_formal_wall_time_seconds": statistics.median(step_times) * round(0.02 / args.dt),
