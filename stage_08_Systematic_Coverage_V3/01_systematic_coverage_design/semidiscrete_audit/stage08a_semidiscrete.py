"""Frozen Stage08A WCSPH semidiscrete and DOP853 audit."""
...
HERE = Path(__file__).resolve(); DESIGN = HERE.parents[1]; ROOT = HERE.parents[3]
for candidate in (DESIGN / "structural_template_library", ROOT / "01_solver"):
...
from stage08a_reference_core import (CS, L, NU, RHO0, SUPPORT_OVER_DX, array_sha, evaluate, exact_frames,
                                     graph_for_positions, minimum_image, output_times, regular_material_layout,
                                     wrap_positions)
...
class Stage08ASemidiscreteRHS:
    def __init__(self, candidate_id: str, labels: np.ndarray, resolution: int) -> None:
        self.candidate_id = candidate_id; self.variant = "MAIN"; self.labels = np.asarray(labels, dtype=np.float64)
        self.resolution = int(resolution); self.count = len(labels); self.dx = L / resolution
        self.support = SUPPORT_OVER_DX * self.dx; self.mass = np.full(self.count, RHO0 * self.dx**2, dtype=np.float64)
        self.mass_t = torch.from_numpy(self.mass); self.accounting = Accounting(); self.sequence = hashlib.sha256()
    ...
    def __call__(self, physical_time: float, state: np.ndarray) -> np.ndarray:
        ...
        pressure_force = conservative_pressure_forces(graph, mass=self.mass_t, density=density_t, pressure=pressure)
        ...
        tau = physical_time * CS / L; source = evaluate(self.candidate_id, self.variant, self.labels, tau)["source"]
        self.accounting.source_evaluation_count += 1
        acceleration = ((pressure_force + viscosity_force) / self.mass_t[:, None]).detach().numpy() + source
        return self.pack(velocity, acceleration, density_rate.detach().numpy())


def integrate(candidate_id: str, resolution: int, rtol: float, atol: float) -> dict[str, Any]:
    labels, _ = regular_material_layout(resolution); _n, tau, physical = output_times()
    exact0 = evaluate(candidate_id, "MAIN", labels, tau[0]); rhs = Stage08ASemidiscreteRHS(candidate_id, labels, resolution)
...
def audit_case(candidate_id: str, resolution: int) -> tuple[dict[str, Any], dict[str, Any]]:
    exact = exact_frames(candidate_id, "MAIN", resolution)
    primary = integrate(candidate_id, resolution, 1e-11, 1e-13)
    sensitivity = integrate(candidate_id, resolution, 1e-12, 1e-14)
    repeat = integrate(candidate_id, resolution, 1e-11, 1e-13)
    scales = {"position": L, "velocity": CS, "density": RHO0, "pressure": RHO0 * CS**2}
    fields = {}; exact_diagnostic = {}
    for key, scale in scales.items():
        delta = minimum_image(primary[key] - sensitivity[key]) if key == "position" else primary[key] - sensitivity[key]
        values = delta / scale; edelta = minimum_image(primary[key] - exact[key]) if key == "position" else primary[key] - exact[key]
        fields[key] = {"normalized_L2": float(np.sqrt(np.mean(values**2))), "normalized_Linf": float(np.max(np.abs(values)))}
        exact_diagnostic[key] = {"normalized_L2": float(np.sqrt(np.mean((edelta/scale)**2))),
                                 "normalized_Linf": float(np.max(np.abs(edelta/scale))),
    ...
    rhs_equal = primary["rhs_graph_sequence_sha256"] == repeat["rhs_graph_sequence_sha256"]
    graph_equal = primary["output_graph_hashes"] == sensitivity["output_graph_hashes"] == repeat["output_graph_hashes"]
    max_l2 = max(row["normalized_L2"] for row in fields.values()); max_linf = max(row["normalized_Linf"] for row in fields.values())
    gates = {"normalized_L2": max_l2 <= 1e-9, "normalized_Linf": max_linf <= 1e-8,
             "primary_repeat_deterministic": repeat_equal and rhs_equal, "graph_sequence_deterministic": graph_equal,
    ...
    metrics = {"candidate_id": candidate_id, "variant": "MAIN", "resolution": resolution,
               "field_primary_sensitivity": fields, "maximum_normalized_L2": max_l2, "maximum_normalized_Linf": max_linf,
               "primary_repeat_bitwise": repeat_equal, "rhs_graph_sequence_repeat": rhs_equal,
               "output_graph_event_sequence_deterministic": graph_equal, "primary_nfev": primary["nfev"],
               "sensitivity_nfev": sensitivity["nfev"], "repeat_nfev": repeat["nfev"],
