#!/usr/bin/env python3
"""Execute the frozen nine-run Stage 02M matrix and one-time sealed test."""
...
sys.path.insert(0, str(ROOT / "postfit_structure"))
sys.path.insert(0, str(LROOT / "loss"))
sys.path.insert(0, str(KROOT / "implementations"))

from controlled_loader import AccessPolicyError, ControlledStage02MLoader  # noqa: E402
...
def make_items(loader: ControlledStage02MLoader, inputs: dict[str, Any], role: str, purpose: str) -> list[DatasetItem]:
    items = []
...
def split_hash(loader: ControlledStage02MLoader, role: str) -> str:
...
def create_optimizer(model: torch.nn.Module) -> torch.optim.AdamW:
    return torch.optim.AdamW(model.parameters(), lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-6)
...
def checkpoint_payload(architecture: str, run_id: str, seed: int, update: int, model: torch.nn.Module, optimizer: torch.optim.Optimizer, scheduler: FrozenPostOptimizerScheduler, best_metric: float, best_update: int, patience_metric: float, patience_update: int, training_history: list[dict[str, Any]], validation_history: list[dict[str, Any]], loader: ControlledStage02MLoader) -> dict[str, Any]:
    return {
        ...
        "normalization_hash": NORMALIZATION_HASH, "run_id": run_id, "training_seed": seed,
        ...
        "provenance": {"stage": "02M", "protocol_hash": PROTOCOL_HASH, "architecture_hash": ARCHITECTURE_HASH, "test_target_access": False},
    }
...
def train_run(architecture: str, seed: int, expected_initial_hash: str, train_items: list[DatasetItem], validation_items: list[DatasetItem], loader: ControlledStage02MLoader) -> dict[str, Any]:
    ...
    for update in range(start_update+1, 1001):
        ...
        for item in train_items:
            ...
            contribution = graph_node_mse(output, item.target) / 10.0; contribution.backward(); total_loss += float(contribution.detach())
    ...
    terminal = {
        ...
        "protocol_hash": PROTOCOL_HASH, "architecture_hash": ARCHITECTURE_HASH, "test_target_access_during_run": False,
    }
    write_json(terminal_path, terminal)
    return terminal


def preflight(loader: ControlledStage02MLoader, freeze: dict[str, Any], environment: dict[str, Any]) -> dict[str, Any]:
    ...
    okay = all(value == "PASS" for value in probes.values()) and not (ROOT / "test_seal/test_release_manifest.json").exists() and loader.audit()["test_target_decode_count"] == 0 and environment["status"] == "PASS"
...
def success_evaluation(run_terminals: list[dict[str, Any]], selected_metrics: dict[str, dict[str, Any]], test_metrics: dict[str, dict[str, Any]], postfit: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {"contract_source": str((LROOT / "success_gates/future_success_gates.json").relative_to(REPO)), "contract_sha256": sha(LROOT / "success_gates/future_success_gates.json"), "architectures": results, "at_least_one_K1_K2_passes_A_through_E": qualified, "status": "PASS" if qualified else "FAIL"}
